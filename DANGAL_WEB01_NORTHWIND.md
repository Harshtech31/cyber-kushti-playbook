...
Attacker carries Rathore's staging session cookie to production, allowing session portability
across subdomains without requiring a new IdP authentication event. The production logs show
Rathore's prior legitimate production sign-in through SSO where 2FA was satisfied (F28); the
attacker's staging-issued cookie was accepted directly by production without triggering an
SSO/2FA re-challenge.
...
Result: Of the 41,900 credit entries in the database, 3,700 were legitimate goodwill credits issued by
named customer service staff for an official product recall announced on 29 July, while the remaining
38,200 entries (requested by `svc:checkout`) were fraudulent (F19).

---

### Phase 6 — Fraud: Forged Payment Callbacks (22 August – 11 September)

**Step 8 — Unsigned Callback Forgery → Orders Released Without Payment** `(F22 → F23 → F24, Section 3)`
...
exfiltration (F22, same host as F16). Each callback carries a real, consecutive order reference
belonging to an order in `awaiting_payment` state harvested via IDOR (F24). Each forged callback
marked the order as paid without any corresponding gateway settlement (F22), causing the warehouse
to pick and ship the goods (Section 3).
...
Session cookies scoped to parent domain (.northwind-goods.com) with no environment binding;
production accepts staging-issued cookies (F29). Attacker presents Rathore's staging session
cookie to production without triggering an SSO/2FA re-challenge (F28).
...
Mass assignment: profile update called 2,840 times with store_credit_paise key (F21). Of 41,900
total credit entries (₹1.91 cr), 3,700 were legitimate recall goodwill credits and 38,200 were
fraudulent entries generated by svc:checkout (F19).

Step 8 — FORGED UNSIGNED PAYMENT CALLBACKS → ORDERS RELEASED (22 Aug – 11 Sep)
Callback handler skips HMAC verification when signature header is absent (F23). Attacker sends
4,118 forged callbacks from EU host IPs with zero gateway settlements (F22) referencing real
awaiting_payment orders harvested via IDOR (F24). Orders marked paid, releasing goods for shipping (Section 3).
```
